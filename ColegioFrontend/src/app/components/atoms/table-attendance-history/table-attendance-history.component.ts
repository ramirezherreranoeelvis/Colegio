...
export class TableAttendanceHistoryComponent {
        @Input() historial: SemanaHistorial[] = null;

        getClasesEntradaSalida(status: string, isFinal: boolean, type: string): string {
                let clases: string = 'history__body-item history__body-date history__body-item--';
                if (isFinal) {
                        if (status === 'asistio') {
                                clases += 'asistencia history__body-item--final-' + type;
